...
// Detectar si un item es follaje
const FOLLAJE_KEYS = ['nube', 'gypsophila', 'follaje', 'eucalipto', 'helecho', 'ruscus', 'hierba', 'pampas', 'palma']
const esFollaje = (nombre: string) =>
  FOLLAJE_KEYS.some(f =>
    nombre.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(f)
  )
...
export default function CotizadorPage() {
    ...
    const cargarTamanos = async () => {
      try {
        ...
        const rescate: TamanoRamo[] = [
          { id: '1', clave: 'S',  nombre: 'Pequeño',      flores_base: 12,  precio_extra: 0,   multiplicador: 1,   descripcion: 'Ideal para detalle', papel_precio: 18  },
          { id: '2', clave: 'M',  nombre: 'Mediano',       flores_base: 24,  precio_extra: 150, multiplicador: 1.5, descripcion: 'Tamaño estándar',    papel_precio: 40  },
          { id: '3', clave: 'L',  nombre: 'Grande',        flores_base: 50,  precio_extra: 300, multiplicador: 2,   descripcion: 'Para impresionar',   papel_precio: 60  },
          { id: '4', clave: 'XL', nombre: 'Extra Grande',  flores_base: 100, precio_extra: 600, multiplicador: 3,   descripcion: 'Máximo impacto',     papel_precio: 100 },
        ]
        setTamanos(rescate)
  ...
  const aprobarCotizacion = () => {
    ...
    const lineasFlores = floresPrincipales.length > 0
      ? floresPrincipales
          .map(i => `• ${i.cantidad}x ${i.flor.nombre} ${i.flor.color}`)
          .join('\n')
      : '• Sin especificar'

    // Follaje — solo si existe (detectado por IA o agregado por cliente)
    const lineasFollaje = follajes.length > 0
      ? follajes.map(i => `• ${i.flor.nombre}`).join('\n')
      : null

    // Accesorios — solo si eligió alguno
    const lineasAccesorios = accesorios.length > 0
      ? accesorios.map(a => `• ${a.emoji} ${a.nombre}`).join('\n')
      : null

    // Entrega
    let seccionEntrega = '📦 Entrega: Por confirmar'
    if (datos.envio?.tipo === 'domicilio') {
      seccionEntrega = [
        '🏠 Envío a domicilio',
        `Zona: ${datos.envio.zona?.nombre}`,
        `Costo: $${datos.envio.precio.toFixed(2)} MXN`,
      ].join('\n')
    ...
    if (lineasFollaje) {
      lineas.push('', '🌿 *Follaje:*', lineasFollaje)
    }

    if (lineasAccesorios) {
      lineas.push('', '🎀 *Accesorios:*', lineasAccesorios)
    }

    lineas.push(
      '',
      `📐 *Tamaño:* ${datos.tamano?.nombre ?? 'Por definir'}`,
      `🎁 *Envoltura:* ${datos.papel?.nombre ?? 'Incluida'}`,
      '',
      seccionEntrega,
    )

    if (datos.nota?.trim()) {
      lineas.push('', `📝 *Nota:*`, datos.nota.trim())
    }

    lineas.push(
      '',
      `💵 *TOTAL A COBRAR: $${datos.total.toFixed(2)} MXN*`,
      '_Sujeto a confirmación_',
      '',
